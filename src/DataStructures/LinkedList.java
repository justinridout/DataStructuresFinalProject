...
public class LinkedList {
	...
	public Registration deleteByClientEmail(String index) {
		...
		if (!this.isEmpty()) {
			...
			// While loop traverses the list checking until correct company is found or list
			// hits the end
			while (traverse.next != null) {
	...
	// traverses the list while adding company names to the prints string. print
	// string is returned
	public String print() {
		String printString = "";
		int count = 1;
	...
	public Registration[] toArray(Trainer t) {
		Registration[] r = new Registration[this.size];
		LinkedNode traverse = this.head;

		int index = 0;
		while (traverse != null) {
			if (traverse.data.getClient().getTrainer().equals(t)) {
				r[index] = traverse.data;
			}
	...
	public Registration searchByClientEmail(String index) {
		...
		if (!this.isEmpty()) {
			...
			// else while loop traverses the loop until the company is found or end of list
			// is hit
			while (traverse.next != null) {
				if (traverse.next.data.getClient().getEmail() == index) {
					data = traverse.next.data;
